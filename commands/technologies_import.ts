 ...
 * missing logo or documentation link is filled in either way.
 */
  ...
  async run() {
    ...
    for (const entry of entries) {
      ...
      if (known && !this.update) {
        ...
        if (filled.length > 0) {
          await known.save()
          this.logger.info(`${entry.slug} · kept, ${filled.join(' and ')} attached`)
        } else {
      ...
      const technology = await Technology.updateOrCreate(
        { slug: entry.slug },
        {
          ...
          docsUrl: entry.docsUrl ?? known?.docsUrl ?? null,
        }
      )
