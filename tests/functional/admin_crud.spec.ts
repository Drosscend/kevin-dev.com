...
test.group('Admin CRUD technologies', (group) => {
  ...
  test('le lien de documentation est enregistré, vidé et validé', async ({ client, assert }) => {
    const user = await admin()
    ...
    const technology = await Technology.findByOrFail('slug', 'react')
    assert.equal(technology.docsUrl, 'https://react.dev/learn')
    ...
    const invalid = await client
      ...
      .withCsrfToken()
      .redirects(0)
      .form({ slug: 'react', name: 'React', category: 'framework', docsUrl: 'pas-une-url' })
    invalid.assertStatus(302)
    await technology.refresh()
    assert.equal(technology.docsUrl, 'https://react.dev/learn', 'une URL invalide est rejetée')
    ...
    const cleared = await client
      ...
      .withCsrfToken()
      .redirects(0)
      .json({ slug: 'react', name: 'React', category: 'framework', docsUrl: null })
    cleared.assertStatus(302)
    await technology.refresh()
    assert.isNull(technology.docsUrl)
  })
})
