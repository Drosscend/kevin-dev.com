...
test.group('Technologies publiques', (group) => {
  ...
  test('la liste groupe les technologies et compte projets et articles publiés', async ({
    client,
    assert,
  }) => {
  ...
  test('la fiche liste les projets et articles publiés qui portent la technologie', async ({
    client,
    assert,
  }) => {
    const adonis = await Technology.create({ slug: 'adonisjs', name: 'AdonisJS' })
